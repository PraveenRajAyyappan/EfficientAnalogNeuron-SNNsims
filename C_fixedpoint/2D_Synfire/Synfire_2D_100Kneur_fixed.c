...
#define MYFILENAME "Synfire_17.txt"
#define INPUTNEURONS 46
#define INPUTNOISE   0.001
#define SimTime      200e3
#define Syn  0.44  // Synapse middlegroup
//#define INPUTNOISE 0.002
...
#define MinusSpikeThresh fi(-1.4542,24)  // 3.2 in Q8.24
...
#define Synapse_Mat_fixed_term  2.1425
...
#define OFF 21
#define Num_Layer NeuronPopulation/Group
#define Num_Synn_Conn (Num_Layer-1)*(Group*Group)

///////////////////// SYNFIRE JITTER ANALYSIS (NEW) //////////////////////
...
double generateGaussianNoise(double mean, double stdDev) {
    ...
    double z = sqrt(-2.0 * log(U1)) * cos(2.0 * M_PI * U2); // Generate Gaussian-distributed random number
    
    return mean + stdDev * z; // Scale and shift to match desired mean and standard deviation
}
...
void InputCurrent_compute( int Triangle_scaled[2*NeuronPopulation], int fix_Inp_current[NeuronPopulation], int InputCurrent[NeuronPopulation],float *noise){
  int fix_temp1;
  int Layer = -1; // Because starting with update
  int Start = 0;
  int Stop = Group;
  for(int i=0; i< Group; i ++)
    InputCurrent[i] = fix_Inp_current[i];
  for(int i=Group; i< NeuronPopulation; i++){
    ...
    for (int j = Start; j < Stop; j++){
            fix_temp1 += two_power_of(mul_16_24_24(EffCoupling,Triangle_scaled[j<<1]) + 0x56000000);
    }
    InputCurrent[i] = mul_24_8_8( mul_24_8_24(fi(Synapse_Mat_fixed_term * pow(2,(((-Kp*(Syn+noise[i]))/(UT*log(2)))+9.9658)),24),0x58),fix_temp1);
  }
}

int Network_ODE(int *fix_dydt, int *y, int fix_Vmem[NeuronPopulation], float *noise, int fix_Inp_current[NeuronPopulation], int fix_Kp, int fix_EXPMEL, int fix_EXPVSAT, int fix_EXPMVGK, int fix_EXPVNA , int fix_EXPEK , int fix_AMAT[2][2], int fix_T_MAT_neu[4][8] ){
  ...
  InputCurrent_compute( fix_Triangle_scaled, fix_Inp_current, fix_InputCurrent, noise);
...
/////// (NEW) //////////
  ...
  // Define Synfire network
  int Count = 1;
 
 // Generate guassian jitter in the  synaspe weights
  double mean = 0.0;   // Mean of the Gaussian distribution
  double stdDev = 0.01; // Standard deviation of the Gaussian distribution

////////////////////////

  // Define simulation parameters
  float dt     = 0.001; // Units (ms)
  int   NSteps = SimTime;  // Total Number of steps to take in the simulation

/////// (NEW) //////////
  //for guassian noise
  int PulseStartIdx[Group] = {};
  int PulseendIdx[Group] = {};

//////////////////////

  int fix_Inp_current[NeuronPopulation] = {0};
  int fix_PulseHeight = fi(40.0,8); 
...
/*
  double noise[Num_Synn_Conn] = {0};
  
  
  for (int i=0; i<Num_Synn_Conn; i ++){

    noise[i] = generateGaussianNoise(mean, stdDev);
...
//  int CurrPrefix = mul_24_8_24(fi(Synapse_Mat_fixed_term * pow(2,(((-Kp*Syn)/(UT*log(2)))+9.9658)),24),0x58);
  ...
  float *noise = (float *)malloc(Num_Synn_Conn * sizeof(float));
  for (int i=0; i<Num_Synn_Conn; i ++){
...
/////// (NEW) //////////
  //0,0.001,0.005,0.006,0.0065 
  stdDev = INPUTNOISE;

  for (int i = 0; i < Group; i++) {
    PulseStartIdx[i] = generateGaussianNoise(mean, stdDev)*1e6 + 50000;
    //PulseStartIdx[i] = 10000;
    PulseendIdx[i] = PulseStartIdx[i] + 1100;
  }

  for(int i = 0; i < NSteps; i++){
    for (int j=0; j<INPUTNEURONS; j++){
      if(i >= PulseStartIdx[j] && i <= PulseendIdx[j])
        fix_Inp_current[j] = fix_PulseHeight;
      else
        fix_Inp_current[j] = 0;  
  }

    // Run the actual ODE (Explicit Euler)
    Network_ODE(fix_dydt, fix_y, fix_Vmem, noise, fix_Inp_current, fix_Kp, fix_EXPMEL, fix_EXPVSAT, fix_EXPMVGK, fix_EXPVNA, fix_EXPEK, fix_AMAT, fix_T_MAT_neu);
...
/*
    ...
    for(int i=0; i<(NeuronPopulation*6);i++){
      if (i == (NeuronPopulation*2 +temp))
      {
        fprintf(fptr,"%f ",(float) fix_y[i]/(1<<24));
        temp += 4;
      }
    }
    fprintf(fptr,"\n");
